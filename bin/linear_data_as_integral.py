...
class Coverage:
    ...
    def set(self, s, chr_sizes):
        self.sorted = sorted(s, key=lambda x: x[0])
        self.starts = sorted(x[0] for x in s)
        self.ends = sorted(x[1] for x in s)
        self.num_reads = len(s)
    ...
    def count(self, f, t):
        return min(bisect.bisect_right(self.ends, t), self.num_reads-1) - \
               min(bisect.bisect_left(self.starts, f), self.num_reads-1)
